	...
	<meta name="viewport" content="width=device-width, initial-scale=1">
      ...
      <div class="row">
		 ...
		 <div>
			...
			<a class="BreadEff" href="studentMessaging.php"> Message Menu</a>
	    </div>
		<br/>
	...
	<table class='table table-bordered table-striped table-hover'>
		...
		<tr>
			<?php
				...
				$stmt = mysqli_prepare($mysqli, "SELECT reply.replyID, reply.sender, reply.messageTitle, reply.contentDetails 
				FROM reply, message
				WHERE reply.messageID = message.MessageID
				AND message.messageID = ?
				AND reply.recipient = ?");
				if($stmt)
				{
					//bind parameters for makers
					mysqli_stmt_bind_param($stmt,"ii", $messageID, $uID);
					
					//execute query 
					mysqli_stmt_execute($stmt);
					
					//bind the results
					mysqli_stmt_bind_result($stmt, $rID, $s, $mTitle, $cD);
