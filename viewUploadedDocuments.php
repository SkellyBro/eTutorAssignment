...
<html lang="en">
  ...
  <body>
	...
	<meta name="viewport" content="width=device-width, initial-scale=1">
       ...
       <div class="row">
		 ...
		 <div>
			...
			<a class="BreadEff" href="viewStudentUploadList.php">Student Upload List &raquo;</a>
			<a class="BreadEff" href="#.php">Student Uploads</a>
	    </div>
		<br/>
	...
	<?php
	//get student ID from previous page
	$sID= $_GET['studentID'];
	...
	if($stmt){
		...
		if (mysqli_stmt_fetch($stmt))
			{
				echo("<h4>These are all the uploads made by $fn $ln</h4>");
			}else{
				return false;
	...
	$stmt = mysqli_prepare ($mysqli, 
			...
			FROM useraccount, documents
			WHERE useraccount.userID=?
			AND useraccount.userID=documents.uploadedBy
			ORDER BY documents.uploadDate");
			if ($stmt)
				{
				//bind variables
				mysqli_stmt_bind_param($stmt, "i", $sID);	
